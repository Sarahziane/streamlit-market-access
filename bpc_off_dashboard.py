...
# 🗺️ ANALYSE RÉGIONALE
# -------------------------
st.subheader("🗺️ Analyse régionale par type d’établissement")

# Données fictives régionales
regions = [
    ...
    "Normandie", "Bourgogne-Franche-Comté", "Centre-Val de Loire", "Pays de la Loire"
]
etablissement_types = ["Tous", "CHU", "CHG", "ESPIC"]

selected_type = st.selectbox("Filtrer par type d’établissement :", etablissement_types)

# Taux d’adoption simulé (aléatoire ou défini)
import numpy as np
np.random.seed(42)
...
region_data = pd.DataFrame({
    "Région": regions,
    "Taux d’adoption (%)": (adoption_par_region * 100).round(1),
    "Type établissement": np.random.choice(etablissement_types[1:], len(regions)) if selected_type != "Tous" else ["Tous"]*len(regions)
})

if selected_type != "Tous":
    region_data = region_data[region_data["Type établissement"] == selected_type]

# Charger les frontières régionales (simplifiées pour performance)
import requests
...
fig_map = px.choropleth(
    ...
    locations="Région",
    color="Taux d’adoption (%)",
    color_continuous_scale="Viridis",
    title="Taux d’adoption simulé par région"
)
